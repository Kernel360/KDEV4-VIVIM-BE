...
public class FileService {
    ...
    public List<FileListResponse> getPostFiles(Long postId) {
        List<File> files = fileRepository.findAllByReferenceIdAndReferenceTypeAndDeletedAtIsNull(postId, ReferenceType.POST);
        return files.stream()
                .map(FileListResponse::from)
                .collect(Collectors.toList());
    }
    public List<FileListResponse> getApprovalFiles(Long approvalId) {
        List<File> files = fileRepository.findAllByReferenceIdAndReferenceTypeAndDeletedAtIsNull(approvalId, ReferenceType.APPROVAL);
        return files.stream()
                .map(FileListResponse::from)
                .collect(Collectors.toList());
    }
