...
public class LinkService {
    ...
    public void createPostLink(Long postId, LinkRequest request) {
        ...
        Link newLink= Link.createLink(request.getTitle(), request.getUrl(), ReferenceType.POST, postId);
        linkRepository.save(newLink);
    }
    public void createApprovalLink(Long approvalId, LinkRequest request) {

        Link newLink= Link.createLink(request.getTitle(), request.getUrl(), ReferenceType.APPROVAL, approvalId);
        linkRepository.save(newLink);
    }

    public List<LinkListResponse> getPostLinks(Long postId) {
        List<Link> links = linkRepository.findAllByReferenceIdAndReferenceTypeAndDeletedAtIsNull(postId, ReferenceType.POST);
        return links.stream()
    ...
    public List<LinkListResponse> getApprovalLinks(Long postId) {
        List<Link> links = linkRepository.findAllByReferenceIdAndReferenceTypeAndDeletedAtIsNull(postId, ReferenceType.APPROVAL);
        return links.stream()
                .map(LinkListResponse::from)
                .collect(Collectors.toList());
    }
