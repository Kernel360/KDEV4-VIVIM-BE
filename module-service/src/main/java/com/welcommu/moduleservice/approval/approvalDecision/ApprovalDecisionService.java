...
public class ApprovalDecisionService {
    ...
    @Transactional
    public void createDecision(User user, Long proposalId, DecisionRequestCreation request) {
        ...
        if (isAdmin(user)) {
            approver = approvalApproverRepository.findFirstByApprovalProposal(proposal).orElseThrow(
